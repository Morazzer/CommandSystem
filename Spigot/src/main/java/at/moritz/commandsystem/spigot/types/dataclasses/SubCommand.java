package at.moritz.commandsystem.spigot.types.dataclasses;

import at.moritz.commandsystem.javacord.Main;
import at.moritz.commandsystem.spigot.logs.Logger;
import at.moritz.commandsystem.spigot.types.annotations.CommandMessage;
import at.moritz.commandsystem.spigot.types.annotations.CommandName;
import at.moritz.commandsystem.spigot.types.annotations.CommandParam;
import at.moritz.commandsystem.spigot.types.annotations.CommandPathParam;
...
public class SubCommand {
    ...
    public SubCommand(Method method, Command command) {
        this.commandPath = Arrays.stream(method.getAnnotations())
                .noneMatch(annotation -> annotation instanceof at.moritz.commandsystem.spigot.types.annotations.SubCommand)
                ? ""
                : Arrays.stream(method.getAnnotations())
                .filter(annotation -> annotation instanceof at.moritz.commandsystem.spigot.types.annotations.SubCommand)
                .map(annotation -> (at.moritz.commandsystem.spigot.types.annotations.SubCommand) annotation)
    ...
    private @Nullable Object parsePathParam(int index, String content, Class<?> clazz, Message message) {
        String fullPath = message.getContent().substring(Main.getPrefix().length() + message.getContent().split(" ")[0].length());
        ...
        if (clazz.equals(String.class)) {
            ...
            try {
                ...
                return Main.getInstance().getApi().getUserById(userid).join();
                ...
                return Main.getInstance().getApi().getUserById(arg.substring(3, arg.length() - 1)).join();
            }
        }
    ...
    private @Nullable Object parseCommandParam(int index, String content, Class<?> clazz, Message message) {
        ...
        if (clazz.equals(String.class)) {
            ...
            try {
                ...
                return Main.getInstance().getApi().getUserById(userArg.substring(3, userArg.length() - 1)).join();
            }
        }
